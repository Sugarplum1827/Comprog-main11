...
namespace PatientInformationSystem {
	...
	using namespace  Mysql::MysqlClient;
...
String^ connString = "Server=localhost;Database=comproooooooog;User Id=root;Password=test;";
...
private: System::Void button3_Click(System::Object^ sender, System::EventArgs^ e) {
	Frontpage^ mainMenu = gcnew Frontpage();
    mainMenu->Show();
    this->Close();
}
...
private: System::Void Submit_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	String^ birthplace = txtbirthpl->Text;
	String^  email= txtemail->Text;
	String^ religion = txreligion->Text;
	String^ civilstatus = txtcivil->Text;
	...
	if(name -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper fullname.");
		isValid = false;
	}
	if(age <= 0){
		MessageBox::Show("Enter proper Age.");
		isValid = false;
	}
	if(gender -> Trim()->lenght <= 0) {
		MessageBox::Show("Enter proper gender.");
		isValid = false;
	}
	if(contact -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper contact.");
		isValid = false;
	}
	if(address -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper address.");
		isValid = false;
	}
	if(birthplace -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper birthplace.");
		isValid = false;
	}
	if(email -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper email.");
		isValid = false;
	}
	if(religion -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper religion.");
		isValid = false;
	}
	if(civilstatus -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper civiltatus.");
		isValid = false;
	}
	if(birthdate -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper birthdate.");
		isValid = false;
	}
	if(bloodtype -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper bloodtype.");
		isValid = false;
	}
	if(diagnosis -> Trim()->lenght <= 2) {
		MessageBox::Show("Enter proper diagnosis.");
		isValid = false;
	}
	...
	if (isValid) {
		...
		String^ cmdString = "INSERT INTO Patients (name, age, gender, contact, address, birthpl, religion, civil, birthdate, blood, email, diagnosis)
VALUES (@Name, @Age, @Gender, @Contact, @Address, @BirthPlace, @Religion, @CivilStatus, @Birthdate, @BloodType, @Email, @Diagnosis)
";

		cmd = gcnew SqlCommand(query, con);
        cmd->Parameters->AddWithValue("@Name", name->Text);
        cmd->Parameters->AddWithValue("@Age", age->Int);
        cmd->Parameters->AddWithValue("@Gender", Gender->Text);
        cmd->Parameters->AddWithValue("@Contact", Contact->Text);
        cmd->Parameters->AddWithValue("@Address", Address->Text);
        cmd->Parameters->AddWithValue("@BirthPlace", birthpl->Text);
        cmd->Parameters->AddWithValue("@Religion", religion->Text);
        cmd->Parameters->AddWithValue("@CivilStatus", civil->Text);
        cmd->Parameters->AddWithValue("@Birthdate", textBox12->Text);
        cmd->Parameters->AddWithValue("@BloodType", blood->Text);
